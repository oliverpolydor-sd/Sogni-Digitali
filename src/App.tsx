...
function AppContent() {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const affiliateId =
        ...
        localStorage.getItem("sogni_affiliate_id") ||
        "";
  ...
  const whatsappLink =
    "https://wa.me/393755880609?text=Vorrei%20iniziare%20la%20mia%20evoluzione%20digitale";
    ...
    <>
      ...
      <div className="min-h-screen text-slate-50 selection:bg-[#00E5FF]/30 selection:text-white relative overflow-x-hidden">
        ...
        <div className="space-bg">
          <motion.div
            className="space-layer-1"
            style={{ y: useTransform(scrollY, [0, 5000], [0, -200]) }}
          />
          <motion.div
            className="space-layer-2"
            style={{ y: useTransform(scrollY, [0, 5000], [0, -500]) }}
          />
          <motion.div
            className="space-layer-3"
            style={{ y: useTransform(scrollY, [0, 5000], [0, -900]) }}
          />
        </div>
        <SunBackground />
        ...
        {!isLinksPage && (
          <header
            className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? "bg-black/80 backdrop-blur-xl border-b border-white/5" : "bg-transparent"}`}
          >
            <div
              className={`max-w-7xl mx-auto px-6 flex items-center justify-between transition-all duration-300 ${isScrolled ? "h-16" : "h-24"}`}
            >
              ...
              <Link
                to="/"
                className="flex items-center gap-3 justify-center md:justify-start flex-1 md:flex-none hover:opacity-80 transition-opacity"
              >
                <img
                  src="/my-logo.png"
                ...
                />
                <span
                  className={`font-display font-bold tracking-widest uppercase transition-all duration-300 hidden md:block ${isScrolled ? "text-lg" : "text-xl"}`}
                >
                  Sogni Digitali
                </span>
              </Link>
              ...
              <nav className="hidden md:flex items-center gap-10">
                ...
                <Link
                  to="/pricing"
                  className="text-sm font-semibold tracking-wider uppercase text-slate-400 hover:text-white transition-colors"
                >
                  {translations[lang as keyof typeof translations].navPricing}
                </Link>
                <Link
                  to="/#contatti"
                  className="text-sm font-semibold tracking-wider uppercase text-slate-400 hover:text-white transition-colors"
                >
                  {translations[lang as keyof typeof translations].navContact}
                </Link>
                  ...
                  <div className="absolute top-full right-0 mt-2 w-24 bg-black/80 backdrop-blur-xl border border-white/10 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                    {["IT", "EN", "FR", "AR"].map((l) => (
                      <button
                        key={l}
                        onClick={() => setLang(l)}
                        className="block w-full text-left px-4 py-2 text-sm text-slate-300 hover:text-white hover:bg-white/5 first:rounded-t-lg last:rounded-b-lg"
                      >
                        {l}
                      </button>
                    ))}
                ...
                <Tooltip
                  content={
                    isLightMode ? "Passa al tema scuro" : "Passa al tema chiaro"
                  }
                >
                  <button
                ...
                <Tooltip content="Scopri i pacchetti">
                  <Link
                    to="/pricing"
                    className="btn-primary px-6 py-3 rounded-full font-semibold text-sm tracking-wider uppercase hover:scale-105 transition-all flex items-center gap-2"
                  >
              ...
              <div className="md:hidden absolute top-full left-0 w-full bg-black/95 backdrop-blur-xl border-b border-white/5 p-6 flex flex-col gap-6">
                <div className="flex flex-col gap-3">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-4 h-4 transition-transform duration-300 ${isMobileServiziOpen ? "rotate-180" : ""}`}
                    />
                  </button>
                  {isMobileServiziOpen && (
                    <motion.div
                    ...
                    >
                      <Link
                        ...
                        className="text-sm font-semibold tracking-wider uppercase text-slate-300 hover:text-white transition-colors"
                      >
                        {
                          translations[lang as keyof typeof translations]
                            .navServices
                ...
                <Link
                  to="/pricing"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="text-sm font-semibold tracking-wider uppercase text-slate-300"
                >
                  {translations[lang as keyof typeof translations].navPricing}
                </Link>
                <Link
                ...
                <div className="flex gap-4">
                  {["IT", "EN", "FR", "AR"].map((l) => (
                    <button
                      ...
                      className={`text-sm font-semibold tracking-wider uppercase ${lang === l ? "text-[#00E5FF]" : "text-slate-500"}`}
                    >
                      {l}
                    </button>
                  ))}
                ...
                <Link
                  to="/pricing"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="bg-gradient-to-r from-[#00E5FF] to-[#00B4D8] text-[#0B1120] px-6 py-3 rounded-full font-semibold text-sm tracking-wider uppercase flex items-center justify-center gap-2"
                >
                  <WhatsAppIcon className="w-4 h-4" />
              ...
              <Routes location={location} key={location.pathname}>
                ...
                <Route
                  ...
                  element={
                    ...
                    >
                      <SEO
                        title="Sogni Digitali | Agenzia Web e Sviluppo AI"
                        description="Agenzia di sviluppo Web, App, E-commerce e Intelligenza Artificiale a Torino. Evolvi il tuo business con Sogni Digitali."
                      />
                      {/* Hero Section */}
                      <section className="relative pt-32 pb-20 px-6 overflow-hidden min-h-[80vh] flex items-center z-10">
                        <div className="max-w-7xl mx-auto w-full relative z-10 grid lg:grid-cols-2 gap-16 items-center">
                          <motion.div {...luxuryReveal}>
                            ...
                            <h1 className="font-display text-5xl md:text-7xl lg:text-8xl font-bold tracking-tighter leading-[1.1] mb-8 relative group pl-2">
                              <span className="text-gradient-cyan relative z-10 inline-block hover:animate-glitch">
                                {
                                  translations[
                                    lang as keyof typeof translations
                                  ].heroTitle1
                              ...
                              <span className="absolute inset-0 text-[#00E5FF] opacity-0 group-hover:opacity-20 group-hover:translate-x-1 -z-10 transition-opacity">
                                {
                                  translations[
                                    lang as keyof typeof translations
                                  ].heroTitle1
                              ...
                              <span className="absolute inset-0 text-[#E9C349] opacity-0 group-hover:opacity-20 group-hover:-translate-x-1 -z-10 transition-opacity">
                                {
                                  translations[
                                    lang as keyof typeof translations
                                  ].heroTitle1
                            ...
                            <div className="flex flex-wrap items-center gap-6">
                              <Link
                                to="/pricing"
                                className="bg-gradient-to-r from-[#00E5FF] to-[#00B4D8] text-black px-8 py-4 rounded-full font-semibold text-sm tracking-wider uppercase hover:scale-105 transition-transform ambient-shadow-cyan flex items-center gap-2"
                              >
                                {
                                  translations[
                                    lang as keyof typeof translations
                              ...
                              <Link
                                to="/services"
                                className="text-sm font-semibold tracking-wider uppercase text-slate-300 hover:text-white transition-colors flex items-center gap-2"
                              >
                                {
                                  translations[
                                    lang as keyof typeof translations
                          ...
                          >
                            <div className="aspect-square relative flex items-center justify-center">
                              <div className="w-full h-full relative z-10">
                                <React.Suspense
                                  fallback={
                                    <div className="w-full h-full border border-white/5 bg-white/5 rounded-3xl flex items-center justify-center text-sm text-slate-500 uppercase tracking-widest animate-pulse">
                                      Loading 3D...
                                    </div>
                                  }
                                >
                      ...
                      <section
                        id="marketing"
                        className="py-20 px-6 relative overflow-hidden z-10"
                      >
                        ...
                        >
                          <div className="mb-20 md:w-2/3">
                            <h2 className="font-display text-4xl md:text-6xl font-bold tracking-tighter mb-6">
                              ...
                              <span className="text-[#E9C349] drop-shadow-[0_0_15px_rgba(233,195,73,0.3)]">
                                {
                                  translations[
                                    lang as keyof typeof translations
                                  ].marketingTitle2
                          ...
                          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                            ...
                            <div className="glass-panel p-8 rounded-3xl relative group overflow-hidden transition-all duration-500 hover:-translate-y-2 border border-white/5 hover:border-[#E9C349]/50 hover:bg-[#E9C349]/5">
                              ...
                              <h3 className="font-display text-xl font-bold mb-3 text-white group-hover:text-[#E9C349] transition-colors">
                                {
                                  translations[
                                    lang as keyof typeof translations
                                  ].mktService1Title
                      ...
                      >
                        <React.Suspense
                          fallback={
                            <div className="absolute inset-0 flex items-center justify-center opacity-50">
                              <div className="w-8 h-8 rounded-full border-2 border-t-[#E9C349] animate-spin"></div>
                            </div>
                          }
                        >
                          <Rocket3D />
                        ...
                        >
                          ...
                          <div className="grid md:grid-cols-3 gap-8 items-center">
                            {getPricingData(lang).packs.map((pack, idx) => {
                              ...
                              return (
                                ...
                                >
                                  <div
                                    className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-2xl -mr-10 -mt-10 ${isEvo ? "bg-[#00E5FF]/10" : "bg-[#E9C349]/10"}`}
                                  />
                                  {isEvo && (
                                  ...
                                  <h3
                                    className={`font-display text-xl font-bold mb-2 ${isEvo ? "text-[#00E5FF] mt-4" : "text-[#E9C349]"}`}
                                  >
                                    {pack.name}
                                  </h3>
                                  <div className="text-4xl font-bold font-display mb-8">
                                    <ScrambleNumber
                                      ...
                                      className={
                                        isEvo ? "text-white" : "text-[#E9C349]"
                                      }
                                    />
                                    {pack.price !== "Talk to us" && pack.price !== "Parla con noi" && pack.price !== "Parlez-nous" && pack.price !== "تحدث إلينا" && (
                                      <span className={`text-3xl ${isEvo ? "text-white" : "text-[#E9C349]"}`}>€</span>
                                    )}
                                  </div>
                                  <ul className="space-y-4 mb-10 flex-grow">
                                    {pack.features.map((feature, i) => (
                                      ...
                                      >
                                        <Check
                                          className={`w-5 h-5 shrink-0 ${isEvo ? "text-[#00E5FF]" : "text-[#E9C349]"}`}
                                        />
                                        <span>{feature}</span>
                                      </li>
                                    ))}
                                  ...
                                  <Link
                                    to="/pricing"
                                    className={`w-full py-4 rounded-xl font-semibold text-sm tracking-wider uppercase transition-all text-center block ${
                                      isEvo
                          ...
                          <div className="mt-16 text-center">
                            <Link
                              to="/pricing"
                              className="inline-flex items-center gap-2 px-8 py-4 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-colors font-semibold tracking-wider uppercase text-sm"
                            >
                              {translations[lang as keyof typeof translations]
                          ...
                          <div className="grid lg:grid-cols-2 gap-16">
                            ...
                            <div className="space-y-10">
                              ...
                              <a
                                ...
                                className="w-full group flex items-center justify-between p-6 rounded-2xl bg-gradient-to-r from-[#00E5FF]/10 to-transparent border border-[#00E5FF]/20 hover:border-[#00E5FF]/50 transition-all duration-300"
                              >
                              ...
                              {/* Contact Form */}
                              <form
                                className="space-y-6 glass-panel p-8 rounded-3xl relative overflow-hidden"
                              ...
                              >
                                {isSuccess && (
                                  <div className="absolute inset-0 bg-black/90 backdrop-blur-sm z-10 flex flex-col items-center justify-center text-center p-8 animate-in fade-in duration-500">
                                    ...
                                    <p className="text-slate-400 text-sm">
                                      Ti risponderemo al più presto per iniziare
                                      la tua evoluzione digitale.
                                    </p>
                                  </div>
                                )}
                                <div className="grid md:grid-cols-2 gap-6">
                                  <div className="space-y-2">
                                    ...
                                    <input
                                      ...
                                      className="w-full bg-transparent border border-[#E9C349]/30 focus:border-[#E9C349] text-white rounded-xl p-4 outline-none transition-colors"
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="relative">
                                    ...
                                    >
                                      ...
                                      >
                                        {formData.business ||
                                          "Seleziona il tipo di business..."}
                                      </span>
                                      <ChevronDown
                                        className={`w-5 h-5 text-[#E9C349] transition-transform duration-300 ${isDropdownOpen ? "rotate-180" : ""}`}
                                      />
                                    </div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <textarea
                                    ...
                                    className="w-full bg-transparent border border-[#E9C349]/30 focus:border-[#E9C349] text-white rounded-xl p-4 outline-none transition-colors resize-none"
                                    placeholder="Come possiamo aiutarti a evolvere?"
                                  ></textarea>
                                </div>
                                ...
                                <Tooltip content="Invia la tua richiesta ora">
                                  <button
                                    ...
                                    className="w-full py-6 btn-epic font-bold text-lg tracking-wider uppercase antialiased disabled:opacity-70 flex items-center justify-center gap-3"
                                  >
                              ...
                              <div className="w-full h-64 md:h-80 rounded-3xl overflow-hidden border border-white/10 relative glass-panel">
                                <iframe
                                  ...
                                  style={{
                                    border: 0,
                                    filter:
                                      "invert(90%) hue-rotate(180deg) contrast(1.2)",
                                  }}
                              ...
                              <div className="glass-panel p-8 rounded-3xl flex-grow flex flex-col justify-between">
                                ...
                                <div className="pt-8 mt-8 border-t border-white/10 flex items-center gap-4">
                                  <Tooltip content="Metti 'Mi piace' su Facebook">
                                    <a
                                      href="https://www.facebook.com/people/Sogni-Digitali/61570776753312/"
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center hover:bg-[#1877F2]/20 hover:text-[#1877F2] transition-colors"
                                    >
                                  ...
                                  <Tooltip content="Seguici su Instagram">
                                    <a
                                      href="https://www.instagram.com/digitalisogni/"
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center hover:bg-[#E1306C]/20 hover:text-[#E1306C] transition-colors"
                                    >
                                  ...
                                  <Tooltip content="Scopri i nostri video">
                                    <a
                                      href="https://www.tiktok.com/@sognidigitali?is_from_webapp=1&sender_device=pc"
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center hover:bg-[#00f2fe]/20 hover:text-[#00f2fe] transition-colors"
                                    >
                ...
                <Route
                  ...
                  element={
                    <motion.div
                      ...
                      exit={{ opacity: 0 }}
                    >
                      <LuminaDetail lang={lang} />
                    </motion.div>
                  }
                />
                <Route
                  path="/portfolio/nexus"
                  element={
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
        ...
        {!isLinksPage && (
          <footer className="footer-glass pt-16 pb-8 px-6 relative z-20">
            <div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
              ...
              <div className="space-y-4">
                <h4 className="font-display font-bold tracking-widest uppercase text-sm text-white mb-4">
                  {
                    translations[lang as keyof typeof translations]
                      .footerUsefulLinks
                  }
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <Tooltip content="Scrivici subito">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-[#25D366]/20 hover:text-[#25D366] text-slate-400 transition-all"
                    >
                      <WhatsAppIcon className="w-4 h-4" />
                    </a>
                  </Tooltip>
                  <Tooltip content="Seguici su Instagram">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-[#E1306C]/20 hover:text-[#E1306C] text-slate-400 transition-all"
                    >
                  ...
                  <Tooltip content="Scopri i nostri video">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-[#00f2fe]/20 hover:text-[#00f2fe] text-slate-400 transition-all"
                    >
                  ...
                  <Tooltip content="Metti 'Mi piace' su Facebook">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-[#1877F2]/20 hover:text-[#1877F2] text-slate-400 transition-all"
                    >
            ...
            <div className="max-w-7xl mx-auto pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap justify-center gap-4 md:gap-6">
                <Link
                  to="/links"
                  className="text-slate-600 hover:text-white text-xs tracking-widest uppercase transition-colors"
                >
                  Link in Bio
                </Link>
                <Link
                ...
                <Link
                  ...
                  className="text-slate-600 hover:text-white text-xs tracking-widest uppercase transition-colors"
                >
                  {translations[lang as keyof typeof translations].terms ||
                    "Termini"}
                </Link>
              </div>
            </div>
